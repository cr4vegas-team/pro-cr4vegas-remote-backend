...
export class UnitHydrantService {
    ...
    async updateOne(dto: UnitHydrantUpdateDto): Promise<UnitHydrantRO> {
        let foundUnitHydrant: UnitHydrantEntity = await this._unitHydrantRepository.findOne(dto.id)
        if (!foundUnitHydrant) {
            throw new NotFoundException(UnitHydrantExceptionMSG.NOT_FOUND);
        }
        let foundUnitHydrantUnitId: UnitHydrantEntity = await this._unitHydrantRepository.createQueryBuilder('units_hydrants')
